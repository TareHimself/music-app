...
export default function ToastContainer(props: ToastContainerProps) {
  ...
  const fadeOutTime = useMemo(() => props.fadeOutTime ?? 500,[props.fadeOutTime])

  const [activeToasts, setActiveToasts] = useState<IActiveToast[]>([]);

  window._addNewToast = useCallback((config: Pick<IActiveToast,'duration' | 'render'>) => {
  ...
  return (
    <div className="react-basic-toast" style={{ "--fade-in-time": fadeInTime.toString() , "--fade-out-time" : fadeOutTime.toString()} as React.CSSProperties}>
        <div className="toast-container" style={props.style}>
          {activeToasts.map((a) => (
            <ToastItem key={a.toastId} data={a} updateToasts={setActiveToasts} emitter={emitter.current} fadeInTime={fadeInTime} fadeOutTime={fadeOutTime} />
          )) }
        </div>
      </div>
  );
